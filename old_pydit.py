...
def on_editor_key(event):
    ...
    if mode == "INSERT":
    ...
    elif mode == "NORMAL" or mode == "VISUAL":
        if mode == "NORMAL":
            if key == "Escape":
                set_mode("TREE")
                update_node()
            ...
            if key.isdigit():
                # Allow building up numbers like 12, but avoid '0' as first char unless after other digits
        ...
        count = int(command_count) if command_count else 1
        command_count = ""  # reset after any command
